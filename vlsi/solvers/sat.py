...
def at_most_one(bool_vars):
    ...
    constraints : list
        z3 constraints.
    """
    constraints = [z3.Not(z3.And(pair[0], pair[1])) for pair in combinations(bool_vars, 2)]
    return constraints
    ...
    for i in range(chip_w):
        for j in range(max_h):
            ...
            for k in range(n):
                ...
                opt.add(z3.Implies(corners[i][j][k],z3.And(i<=chip_w-new_inst_x[k],j<=chip_h-new_inst_y[k])))
                # Append constraints
